...
class User(db.Model):
    ...
    volunteer_request = db.relationship(
        "VolunteerRequest", backref="user", lazy=True
    )
...
class Cat(db.Model):
    ...
    transport = db.relationship(
        "CatTransport", backref="cat", lazy=True, uselist=False
    )
...
class CatTransport(db.Model):
    ...
    cat_id = db.Column(
        db.Integer, db.ForeignKey("cat.id"), unique=True, nullable=False
    )
...
class AdoptionForm(db.Model):
    ...
    review = db.relationship(
        "AdoptionFormReview", backref="form", uselist=False
    )
...
class AdoptionFormReview(db.Model):
    ...
    form_id = db.Column(
        db.Integer,
        db.ForeignKey("adoption_form.id"),
        unique=True,
        nullable=False,
    )
...
class VolunteerRequest(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    review = db.relationship(
        "VolunteerRequestReview", backref="request", uselist=False
    )
...
class VolunteerRequestReview(db.Model):
    ...
    request_id = db.Column(
        db.Integer,
        db.ForeignKey("volunteer_request.id"),
        unique=True,
        nullable=False,
    )
